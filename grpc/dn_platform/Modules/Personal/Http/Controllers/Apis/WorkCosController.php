...
class WorkCosController extends Controller
{
    /**
     * 通过cdn上传作业
     */
    public function cdnSaveWork(WorkCdnRequest $request)
    {
        ...
        if ($request->hasFile('image_cover')) {
            ...
            ($lesson = CourseLesson::where('id', $request->lesson_id)->with('cover')->first())) {
            $insert_data['image_cover'] = $lesson->cover ? $lesson->cover->filename : '';
        }
