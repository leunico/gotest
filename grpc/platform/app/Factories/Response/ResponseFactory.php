...
class ResponseFactory implements ResponseInterface
{
    use Transform;
     ...
     */
    public function success($data = 'Success.', array $other = []): JsonResponse
    {
        if ($data instanceof LengthAwarePaginator) {
            return $this->success($this->transformPaginator($data, '', $other));
        }

        return response()->json(is_array($data) ? array_merge($data, $other) : $data, Response::HTTP_OK, [], JSON_UNESCAPED_UNICODE);
    }
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function error($message = 'Fail.', int $statusCode = Response::HTTP_BAD_REQUEST): JsonResponse
    {
    ...
    */
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function errorForbidden(string $message = '', $data = []): JsonResponse
    {
        $res = [
            'message' => $message,
            'data' => $data,
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function errorUnprocessableEntity(string $message = '', $data = []): JsonResponse
    {
        $res = [
            'message' => $message,
            'data' => $data,
